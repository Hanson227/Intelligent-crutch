...
int main(void)
{
	...
	{		
		...
		if((help_time>5)||note_flag)
		{

			printf("报警！");
			u2_printf("5555555555");//报警语音提示命令串，防止报警语音接收不到
			if(Save_Data.isParseData)
			{
				SIM800A_Note_Edit(EMERGENCY_CALL_CMD);
				SIM800A_printf("help me!\r\n%s",gps_data);
				note_flag=SIM800A_Note_Send(30);
			}
			else
			{
				SIM800A_Note_Edit(EMERGENCY_CALL_CMD);
		...
		{ 
			...
			{   			 
				printf("建立TCP连接成功\r\n");//串口输出信息
				u2_printf("6666666666");//网络连接提示命令串，防止报警语音接收不到
